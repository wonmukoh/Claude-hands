...
| 엔진 | 보는 것 | COM 필요 | 쓰는 때 |
|---|---|---|---|
| `office` | 열린 문서의 개체 모델 (슬라이드·문단·셀) | 필요 | PowerPoint · Word · Excel |
| `uia` | UI Automation 트리 (그려진 모든 요소) | 필요 | 기본 |
| `win32` | 실제 창 핸들을 가진 컨트롤만 | 불필요 | UIA 를 못 쓸 때 |

기본값 `auto` 는 대상이 Office 프로그램이면 `office` 를, 아니면 `uia` 를 쓰고,
UIA 자체가 뜨지 않는 환경(잠긴 PC, 타입 라이브러리 로드 실패)에서만 `win32` 로
내려갑니다. 어떤 엔진이 실제로 처리했는지는 모든 결과에 표시됩니다
(`win32:invoke`, `office:value.SetValue` 처럼).

`office` 엔진이 따로 있는 이유는 UIA 로는 **문서를 편집할 수 없기** 때문입니다.
PowerPoint 는 슬라이드를 값 패턴 없는 `custom` 으로, 슬라이드 위 텍스트 상자를
`image` 로 내놓습니다. 화면에 그려진 것만 보는 한 슬라이드에 글자를 넣을 방법이
없습니다. 대신 프로그램이 자기 매크로에 열어 주는 개체 모델에 직접 붙으면,
화면을 전혀 거치지 않으므로 **최소화된 창의 슬라이드에도 글자가 들어갑니다.**

```python
app = attach(process="POWERPNT")        # auto → office
app.minimize()
box = app.find("TextBox 4", role="edit")[0]
app.type(box.ref, "최소화 상태에서 쓴 문장")   # office:value.SetValue
```

`win32` 엔진은 소유자 그리기 방식의 최신 앱 내부는 보지 못하지만, 고전 Win32
프로그램에는 충분하고 COM 을 전혀 건드리지 않습니다.
...
Claude: list_windows(process="notepad")
        ...
        menu_select(path="파일 > 저장")
        → menu_select 완료 — 파일 > 저장 (방식: menu:WM_COMMAND)
```

창은 끝까지 최소화된 채였고, 사용자의 마우스와 키보드는 한 번도 뺏기지 않았습니다.

저장 같은 **명령은 `press_keys("ctrl+s")` 대신 `menu_select` 를 쓰세요.** 단축키는
프로그램이 실제 키보드 상태(`GetKeyState`)를 확인해서 처리하는데, 이 도구는 진짜
키를 누르지 않으므로 단축키가 무시되는 경우가 많습니다. 아래 '한계' 참고.
...
* **단축키는 믿을 수 없습니다.** `press_keys("ctrl+s")` 같은 수식키 조합은 프로그램이 `GetKeyState` 로 실제 키보드 상태를 확인해 처리합니다. 이 도구는 진짜 키를 누르지 않으므로(그게 목적입니다) 단축키가 무시되는 경우가 많습니다. Enter·Tab·방향키·문자처럼 `WM_KEYDOWN`/`WM_CHAR` 를 직접 처리하는 키는 동작합니다. 명령을 실행하려면 `menu_select` 나 버튼 클릭을 쓰세요.
* **동작 성공 ≠ 원하는 결과.** 클릭은 "명령을 전달했다"는 뜻이지 "앱이 원하는 일을 했다"는 뜻이 아닙니다. Windows 는 이를 구분할 방법을 주지 않습니다(실측 결과 `SendMessage` 반환값은 실제 효과와 무관했습니다). 중요한 동작 뒤에는 `snapshot` 이나 `wait_for` 로 결과를 확인하세요.
* **최소화 상태를 싫어하는 앱.** 일부 프로그램은 최소화되어 있으면 특정 명령을 무시합니다(실측: winecfg 의 Cancel). 그럴 때는 `control_window(action="restore")` 로 활성화 없이 복원한 뒤 조작하세요.
* **메뉴 읽기.** `menu_select` 는 창의 실제 메뉴에서 명령 ID 를 찾아 `WM_COMMAND` 를 보냅니다. 메뉴가 없거나 다른 프로세스에서 읽을 수 없으면 UI 트리 경로로 자동으로 내려갑니다.
...
실제로 떠 있는 창을 상대로 전 계층을 확인하는 하네스가 들어 있습니다.

```bash
python examples/verify_live.py --process notepad          # 아무 프로그램이나
python examples/verify_live.py --process POWERPNT         # PowerPoint
python examples/verify_pptx.py --write                    # PowerPoint 전용
```

매 단계마다 `GetCursorPos` 와 `GetForegroundWindow` 를 다시 확인해서, 커서나 전경
창이 움직였으면 그 단계를 실패로 기록합니다. 빼앗지 않는 것이 이 도구의 존재
이유이기 때문입니다.

### 이 저장소에서 실제로 확인한 것

Linux 컨테이너에서 Wine 위에 Windows CPython 3.11 을 올리고, 실행 중인 Win32
프로그램(메모장·winecfg)을 상대로 돌린 결과입니다. **14/14 통과**:

| 확인한 것 | 결과 |
|---|---|
| 창 열거·제목·클래스·PID·좌표 | 통과 |
| 연결이 전경 창을 바꾸지 않음 | 통과 |
| 최소화 상태에서 UI 트리 읽기 (보임과 동일) | 통과 |
| **최소화 상태에서 한글 입력 → 바이트 단위 동일하게 되읽기** | 통과 |
| **최소화 상태에서 버튼 클릭 → 실제 대화상자 열림** | 통과 |
| 열린 대화상자를 찾아 Cancel 클릭 → 닫힘 | 통과 |
| 보이는 창·최소화된 창 캡처에 실제 내용 존재 | 통과 |
| 캡처 후 창 크기·위치·최소화 상태 그대로 | 통과 |
| 커서·전경 창을 한 번도 건드리지 않음 | 통과 |

이 과정에서 잡은 실제 버그들: Windows 콘솔 코드페이지에서 한글 출력이 즉시
`UnicodeEncodeError` 로 죽던 문제, 최소화 캡처가 사용자 창을 320x240 으로
영구히 줄여 버리던 문제, 새까만 캡처를 성공으로 보고하던 문제, 단축키 표시
`&` 가 이름에 새어 나와 검색이 실패하던 문제, 그리고 `win32` 엔진의 창 메시지를
COM 워커 스레드로 넘겨 `RPC_E_CANTCALLOUT_ININPUTSYNCCALL` 로 조용히 좌표
클릭으로 내려가던 문제.

Windows COM/UIA 경로는 Wine 에 클라이언트 타입 라이브러리가 없어 여기서
검증하지 못했습니다. 실제 Windows 에서 `claude-hands doctor` 와 위 하네스로
확인하세요.
...
src/claude_hands/
  ...
  win32/controls.py  창 메시지 폴백 엔진 (HWND 컨트롤 → 같은 패턴 프로토콜)
