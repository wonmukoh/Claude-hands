...
기본값 `auto` 는 UIA 를 먼저 쓰고, UIA 자체가 뜨지 않는 환경(잠긴 PC, 타입
라이브러리 로드 실패)에서만 `win32` 로 내려갑니다. `win32` 엔진은 소유자 그리기
방식의 최신 앱 내부는 보지 못하지만, 고전 Win32 프로그램에는 충분하고 COM 을
전혀 건드리지 않습니다. 어떤 엔진이 실제로 처리했는지는 모든 결과에 표시됩니다
(`win32:invoke`, `uia:value.SetValue` 처럼).
